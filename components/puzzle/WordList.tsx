...
export function WordList({ placements, foundWords = new Set(), highlightWords = true, className, compact = false }: WordListProps) {
    ...
    return (
        <div className={className}>
            {!compact && (
                <h3 className="text-xs font-bold uppercase tracking-wider mb-3 pb-2 border-b border-border text-text-primary font-mono">
            ...
            <ul className={useDoubleColumn ? "grid grid-cols-2 gap-x-2 gap-y-2" : "space-y-2"}>
                {words.map((word) => {
                    ...
                    return (
                        <li
                            key={word}
                            className={`font-mono text-xs py-2 px-3 rounded-lg border transition-all ${
                                found
                                    ? 'border-accent bg-accent/20 text-accent line-through'
            ...
            {foundWords.size === words.length && words.length > 0 && !compact && (
                <div className="mt-4 text-center p-3 bg-accent rounded-xl text-black">
